...
{
	return GET_FILE(instance)->get_size(fb2k::noAbort); // -1 when unknown
}

sInt64_t f_foobar2000_getPos(void* instance)
{
	return GET_FILE(instance)->get_position(fb2k::noAbort); // -1 when unknown
}
...
class input_ofr : public input_stubs
{
public:
	input_ofr() : m_instance(OptimFROG_createInstance()) {}
	...
	void open(file_ptr filehint, const char* path, t_input_open_reason reason, abort_callback& abort)
	{
		...
		m_ofs = path != nullptr && pfc::string_extension(path).toLower() == "ofs";
...
FB2K_SERVICE_FACTORY(album_art_reader)
FB2K_SERVICE_FACTORY(album_art_writer);
